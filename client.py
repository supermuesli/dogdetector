...
def main():
	...
	dogs = vae.transform([('/home/kashim/Downloads/dogsncats/dogs/' + ('%d' % i) + '.jpg' ) for i in range(9000, 9500)])
	cats = vae.transform([('/home/kashim/Downloads/dogsncats/cats/' + ('%d' % i) + '.jpg' ) for i in range(9000, 9500)])
	cars = vae.transform([('/home/kashim/Downloads/dogsncats/cars/' + ('%d' % i).zfill(5) + '.jpg' ) for i in range(1, 501)])


	# show features learned
	...
	plt.scatter(Y[:, 0], Y[:, 1], c=['blue']*500 + ['red']*500 + ['green']*500)
	plt.show()


	# autoencoder results
	grid = torchvision.utils.make_grid(vae(dogs), nrow=20)
	plt.imshow(vae.untransform(grid))
	plt.show()

	grid = torchvision.utils.make_grid(vae(cats), nrow=20)
	plt.imshow(vae.untransform(grid))
	plt.show()

	grid = torchvision.utils.make_grid(vae(cars), nrow=20)
	plt.imshow(vae.untransform(grid))
	plt.show()
